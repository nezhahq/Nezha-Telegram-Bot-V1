...
- **API Token / MCP 助手**：可使用 Nezha API Token 绑定面板，并在私聊中通过 `/chat` 准备批量命令执行，确认后经 Dashboard MCP `server.exec` 执行。
...
- `/cron` - 执行计划任务。
- `/services` - 查看服务状态总览。
- `/dashboard` - 管理面板、API Token 和 LLM 设置。
- `/mcp` - 快速进入当前面板的 MCP/LLM 设置。
- `/chat` - 私聊中使用 LLM 运维助手，命令执行需要二次按钮确认。
